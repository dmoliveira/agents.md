...
- recommended value for this repo: `dmoliveira/agents.md`
- keep worktree and branch variation inside the same repo scope unless a future repo-specific reason requires a split
- prefer a repo-local `.codememory/config.yaml` with `defaults.scope_key: dmoliveira/agents.md` so `oc current` and related commands do not silently fall back to `local`
