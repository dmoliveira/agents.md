...
- `make` (entrypoint): `make help`, then run project targets instead of ad-hoc scripts.
  - In this repo: `make preflight`, `make wiki-status`, `make wiki-mirror-status`, `make wiki-sync-check`, `make wiki-sync-dry-run`, `make wiki-sync-apply`, `make wiki-publish-checklist`, `make wiki-probe-dispatch`.
