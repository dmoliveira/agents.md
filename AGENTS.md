...
2) **Reserve edit surface** (Mail)
   - `file_reservation_paths(project_key, agent_name, ["src/**"], ttl_seconds=3600, exclusive=true, reason="ai-knowledge-index-123")`
3) **Announce start** (Mail)
   - `send_message(..., thread_id="ai-knowledge-index-123", subject="[ai-knowledge-index-123] Start: <short title>", ack_required=true)`
4) **Work and update**
   - Reply in-thread with progress and attach artifacts/images; keep the discussion in one thread per issue id
5) **Complete and release**
   - `br close ai-knowledge-index-123 --reason "Completed"` (br is status authority)
   - `release_file_reservations(project_key, agent_name, paths=["src/**"])`
   - Final Mail reply: `[ai-knowledge-index-123] Completed` with summary and links

Mapping cheat-sheet
- **Mail `thread_id`** ↔ `ai-knowledge-index-###`
- **Mail subject**: `[ai-knowledge-index-###] …`
- **File reservation `reason`**: `ai-knowledge-index-###`
- **Commit messages (optional)**: include `ai-knowledge-index-###` for traceability

Event mirroring (optional automation)
- On `br update --status blocked`, send a high-importance Mail message in thread `ai-knowledge-index-###` describing the blocker.
...
- Always include `ai-knowledge-index-###` in message `thread_id` to avoid ID drift across tools.
